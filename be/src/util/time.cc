...
string impala::ToStringFromUnixMicros(int64_t us, const cctz::time_zone* tz,
    TimePrecision p) {
  DCHECK(tz != nullptr);
  chrono::system_clock::time_point t = TimepointFromUnixMicros(us);
  static const char* fmt_millisec = "%Y-%m-%d %H:%M:%E3S";
  static const char* fmt_microsec = "%Y-%m-%d %H:%M:%E6S";
  static const char* fmt_nanosec = "%Y-%m-%d %H:%M:%E9S";
  static const char* fmt_sec = "%Y-%m-%d %H:%M:%S";
  const char* fmt = (p == TimePrecision::Millisecond) ? fmt_millisec :
      (p == TimePrecision::Microsecond) ? fmt_microsec :
      (p == TimePrecision::Nanosecond) ? fmt_nanosec : fmt_sec;
