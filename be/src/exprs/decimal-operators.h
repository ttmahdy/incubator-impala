...
class DecimalOperators {
 ...
 private:
  ...
  /// local time in 'local_tz' time-zone). Rounds instead of truncating if 'round' is true.
  template <typename T>
  static TimestampVal ConvertToTimestampVal(
      const T& decimal_value, int scale, bool round, const cctz::time_zone* local_tz);
