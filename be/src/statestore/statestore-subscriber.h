...
class StatestoreSubscriber {
 public:
  ...
  //
  ///   heartbeat_address - the local address on which the subscriber service which
  ///                       communicates with the statestore should be started.
  //
  ///   statestore_address - the address of the statestore to register with
  //
  ///   rpc_mgr - the RpcMgr to register the subscriber service with.
  ...
  /// Registers this subscriber with the statestore, and starts the subscriber service, as
  /// well as a thread to check for failure and initiate recovery mode.
  //
 ...
 private:
  ...
  /// Address that the subscriber service should be started on.
  ...
  /// RpcMgr for obtaining proxies to the statestore, and for registering the local
  /// subscriber service. Not owned by this class.
  RpcMgr* rpc_mgr_;
  ...
  /// Exit flag - when set the subscriber will exit.
  bool is_shutdown_ = false;

  /// Subscriber service implementation, needs to access UpdateState
  friend class StatestoreSubscriberService;
